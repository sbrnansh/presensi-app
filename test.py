    ...
    with col_kiri:
        ...
        if foto_input is not None:
            ...
            with st.spinner("Memproses presensi..."):
                t0 = timer_module.perf_counter()
                results = model(frame)
                t1 = timer_module.perf_counter()
            ...
            if len(detections) == 0:
                ...
                if spoofing_terdeteksi:
                    ...
                    t4 = timer_module.perf_counter()
                    face_locations = face_recognition.face_locations(rgb_frame)
                    face_encodings = face_recognition.face_encodings(rgb_frame, face_locations)
                    t5 = timer_module.perf_counter()

                    with st.expander("Waktu Proses (debug)", expanded=False):
                        st.write(f"Deteksi wajah (YOLO): {(t1 - t0) * 1000:.1f} ms")
                        st.write(f"Deteksi spoofing (YOLO): {(t3 - t2) * 1000:.1f} ms")
                        st.write(f"Face recognition (dlib): {(t5 - t4) * 1000:.1f} ms")
                        st.write(f"**Total: {(t1 - t0 + t3 - t2 + t5 - t4) * 1000:.1f} ms**")
    ...
    with tab1:
        ...
        def render_training_chart(results_path, nama_model, map_test):
            ...
            fig = px.line(
                ...
                markers=False,
                labels={"epoch": "Epoch", col_map_col: "mAP@50"}
            )
            fig.update_layout(
                height=280,
                margin=dict(t=10, b=10, l=10, r=10),
            ...
            mc1.metric("mAP@50 terbaik", f"{map_test:.4f}", help="Dihitung pada test set")
            mc2.metric("Epoch terbaik", best_epoch)
        ...
        st.caption("Grafik menunjukkan performa model pada data validasi selama 50 epoch pelatihan (100 epoch untuk model spoofing). Nilai pada kartu merupakan hasil evaluasi akhir pada test set.")
        ...
        st.caption("Confusion matrix menggambarkan perbandingan antara nilai prediksi model dengan nilai aktual pada test set.")
